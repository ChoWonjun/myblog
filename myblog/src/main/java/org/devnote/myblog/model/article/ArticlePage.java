...
public class ArticlePage implements Page {
	...
	private String category; // 목록에 포함된 글 객체의 카테고리
	...
	/*
	 * 다음 변수들은 가독성을 위해 선언했습니다. isFirstPage는 현재 페이지가 1페이지면 true를 반환하지만
	 ...
	 * << 생성자 >>
	 * 
	 * 페이징 처리하기위해 반드시 필요한 변수는 페이징 처리할 리스트의 전체 행 갯수(totalListSize) 입니다. 이 변수가 없으면
	 * 페이징 연산이 불가능하기 때문에 디폴트 생성자를 만들지 않음으로서 Paging 객체 생성시 tatalListSize를 반드시 초기화
	 * 하도록 구현했습니다.
	 * 
	 * 현재 페이지(currentPage), 한 페이지당 보여줄 행의 갯수(pageSize), 한 페이지 그룹에 들어갈 페이지의
	 * 갯수(pageGroupSize)는 setter 메소드와 생성자로 변경 할 수 있게 구현했습니다.
	 * 
	 */
	...
	/**
	 * 페이징 처리할 리스트의 전체 행 수를 아규먼트에 넘겨주면 페이징 처리가 완료됩니다. 현재 페이지는 1로 초기화됩니다. 한 페이지당
	 * 보여줄 행의 갯수는 기본값 5으로 초기화됩니다. 한 페이지 그룹에 들어갈 페이지의 갯수는 기본값 5로 초기화됩다.
	 * 
	 * @param totalListSize
	 */
	public ArticlePage(String category, int totalListSize) {
	...
	/**
	 * 현재 페이지와 페이징 처리할 리스트의 전체 행 수를 아규먼트에 넘겨주면 페이징 처리가 완료됩니다. 한 페이지당 보여줄 행의 갯수는
	 * 기본값 5으로 셋팅됩니다. 한 페이지 그룹에 들어갈 페이지의 갯수는 기본값 5로 셋팅됩니다.
	 * 
	 * @param currentPage
	 * @param totalListSize
	 */
	public ArticlePage(String category, int currentPage, int totalListSize) {
		this.category = category;
	...
	/**
	 * 
	 * 현재 페이지, 전체 행 수, 한 페이지당 보여줄 행 수, 한 페이지 그룹에 보여줄 페이지 수를 아규먼트에 넘겨주면 페이징 처리가
	 * 완료됩니다.
	 * 
	 * @param currentPage
	 * @param pageSize
	 ...
	 * @param totalListSize
	 */
	public ArticlePage(String category, int currentPage, int pageSize, int pageGroupSize, int totalListSize) {
		this.category = category;
		this.currentPage = currentPage;
	...
	private void calculate() {
		...
		if (currentPage <= 0)
			currentPage = 1; // 현재 페이지가 0 이하일때 1로 초기화

		if (currentPage > pageCount)
			currentPage = pageCount; // 현재 페이지가 페이지 갯수를 초과할때 마지막 페이지로 할당
	...
	public String toString() {
		return "\n\nPaging [totalListSize=" + totalListSize + ", pageSize=" + pageSize + ", pageGroupSize="
				+ pageGroupSize + ", pageCount=" + pageCount + ", pageGroupCount=" + pageGroupCount + "\n ,currentPage="
				+ currentPage + ", currentPageGroup=" + currentPageGroup + ", beginRow=" + beginRow + ", endRow="
				+ endRow + ", beginPage=" + beginPage + ", endPage=" + endPage + "\n" + ", isFirstPage=" + isFirstPage
				+ ", isLastPage=" + isLastPage + ", isFirstPageGroup=" + isFirstPageGroup + ", isLastPageGroup="
				+ isLastPageGroup + "]";
	}
